...
\begin{hypH}
\label{assum:moments}
For all $1\leq t \leq T$, $y_{0:T}$,
$$
\mu\otimes\mu(K^{y_{0:T}}_{t-1|t})<\infty\eqsp,
$$
where $K^{y_{0:T}}_{t-1|t}$ is defined in H\ref{assum:lip}.
\end{hypH}
...
Assume that H\ref{assum:transition:hmm} and H\ref{assum:bound:likelihood} hold. For all $\theta\in\Theta$,  all $t\geq 0$, all $y_{0:t}$,
$$
...
\end{lemma}
\begin{proof}
Note that by \eqref{eq:def:addfunc} $h^{y_{0:T}}_{t,\theta,\varphi}(X_{t-1},X_t) = \log q^{y_{0:T}}_{\varphi,t-1|t}(X_t,X_{t-1}) - \log b^{y_{0:t-1}}_{\theta,t-1|t}(X_t,X_{t-1})$ and, by convention,  $h^{y_{0:T}}_{T,\theta,\varphi}(X_{T-1},X_T) = \log q^{y_{0:T}}_{\varphi,T}(x_T) - \log \phi^{y_{0:T}}_{\theta,T}(x_T)$. 
\end{proof}
