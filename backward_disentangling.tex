...
\begin{itemize}
\item
Consider first the linear and Gaussian case, i.e. we assume that for all $x_k$, $\hd{k+1;\parvec}(x_{k}, \cdot)$ is the Gaussian pdf with mean $A_k^\theta x_k$ and variance $R_k^\theta$  and that $\md{k;\parvec}(x_{k},\cdot)$ is the Gaussian pdf with mean $B_k^\theta x_k$ and variance $S_k^\theta$. In this setting, assume that $\phi_k^\varphi$ is the Gaussian pdf with mean $\mu_k^\varphi$ and variance $\Sigma_k^\varphi$ and that for all $x_k$, $q_{\varphi,k-1}(x_{k}, \cdot)$ is the Gaussian pdf with mean $A_k^\varphi x_k$ and variance $R_k^\varphi$. In this case, for all bounded and measurable function $h$,
$$
...
$$ 
where $\|\cdot\|_{\mathrm{tv}}$ is the total variation distance and $\mu_{k-1:k}^\varphi$  (resp. $\mu_{k-1:k}^{\varphi,\theta}$) is a short-hand notation for the joint distribution $\post{k}^{\varphi}Q_{\varphi,k-1} $ (resp. $\post{k-1}^{\varphi} \uk{k-1,\theta}h/\post{k-1}^{\varphi} \uk{k-1,\theta}\1$). Therefore, we can choose $c_k(\theta,\varphi) = 2 \left\|\mu_{k-1:k}^\varphi - \mu_{k-1:k}^{\varphi,\theta}\right\|_{\mathrm{tv}}$. It remains to use the fact that $\mu_{k-1:k}^\varphi$  and $\mu_{k-1:k}^{\varphi,\theta}$ are Gaussian pdf and that $ \left\|\mu_{k-1:k}^\varphi - \mu_{k-1:k}^{\varphi,\theta}\right\|_{\mathrm{tv}} \leqslant (\mathrm{KL}(\mu_{k-1:k}^\varphi \| \mu_{k-1:k}^{\varphi,\theta})/2)^{1/2}$ and that we have an explicit expression of the $\mathrm{KL}$ divergence between Gaussian  distributions.
\item Nonlinear ICA ?
\end{itemize}
