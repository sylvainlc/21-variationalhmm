...
\textcolor{red}{Discuter le papier de Doucet, le fait que c'est naturel pour les HMM et que \c ca permet l'estimation en ligne : mais que ce n'est pas gratuit. Et ajouter qu'aucun r\'esultat sur la qualit\'e de l'approximation dans l'espace latent n'a \'et\'e obtenu \`a ce jour.}
 ...
 %On the other hand, for a fixed record of observations $Y_{0:n}$, for all $0 \leq k \leq n-1$,   $ \post{k}^{\parvar}$ is the marginal distribution of $ \post{0:n}^{\parvar}$ at time $k$ which therefore targets the marginal smoothing distribution at time $k$, i.e. the distribution of $X_k$ given $Y_{0:n}$.
...
%In this section we propose an upper bound for the error $|\mathbb{E}_{q_\parvar}[\af{0:n}(X_{0:n})]-\mathbb{E}^\parvec[\af{0:n}(X_{0:n})|Y_{0:n}]|$  when $\af{0:n}$ is an additive functional of the hidden states as in \eqref{eq:additive:functional}. %Following \cite{gloaguen2019pseudo} this control can be obtained in a broader framework than the HMM setting. Let $(\Xset_n, \Xfd_n)_{n \in \nset}$ be a sequence of general state spaces and let, for all $n \in \nset$ and all $\parvec$, $\uk{n,\parvec}$ defined on  $\Xset_n \times \Xfd_{n + 1}$ be bounded kernels in the sense that $\sup_{x \in \Xset_n} \uk{n,\parvec}(x, \Xset_{n + 1}) < \infty$. We will assume a dominated model where each kernel $\uk{n,\parvec}$ has a kernel density $\ud{n,\parvec}$ with respect to some $\sigma$-finite reference measure $\mu_{n + 1}$ on $\Xfd_{n + 1}$. Finally, let $\chi$ be some bounded measure on $\Xfd_0$. In the following, we denote state-space product sets and $\sigma$-fields by $\Xset^n \eqdef \Xset_0 \times \cdots \times \Xset_n$ and $\Xfd^n \eqdef \Xfd_0 \tensprod \cdots \tensprod \Xfd_n$, respectively, and consider probability measures  
%\begin{equation} \label{eq:def:post}
%\post{0:n}^\parvec(\rmd x_{0:n}) \propto \chi(\rmd x_0) \prod_{m = 0}^{n - 1} \uk{m,\parvec}(x_m, \rmd x_{m + 1}), \quad n \in \nset, 
%\end{equation}
%on these product spaces.\footnote{We will always use the standard convention $\prod_{\varnothing} = 1$, implying that $\post{0} \propto \chi$.}   the HMM setting discussed in this sectionIn the special case where each $\uk{n,\parvec}$ can be decomposed as $\uk{n,\parvec}(x_n, \rmd x_{n + 1}) = \md{n,\parvec}(x_n) \, \hk_{n,\parvec}(x_n, \rmd x_{n + 1})$ for some Markov kernel $\hk_{n,\parvec}$ and some nonnegative potential function $\md{n,\parvec}$, \eqref{eq:def:post} yields. 

%\paragraph{Variational family. }
%For all $n\in\nset$, without loss of generality, write $\post{0:n}^{\parvar}$ the joint smoothing distribution provided by the variational family. Then,
%$$
 %\post{0:n}^{\parvar}(x_{0:n})=  \post{n}^{\parvar}(x_n)\prod_{k=0}^{n-1}q_{\parvar,k}(x_{k+1},x_k)\eqsp.
%$$
%with the convention $\post{0:n}^{\parvar}(x_{n}) = \post{n}^{\parvar}(x_{n})$. 
 %\textcolor{red}{Here $q_{\parvar,k+1}(dx_{k+1})$, $q_{\parvar,k}(dx_{k})$ and $q_{\parvar,n}(dx_0)$ are not really $ q_{\parvar,\ell}$ but the marginals  of $ \post{0:n}^{\parvar}$ ?}

\begin{hypH}
\label{assum:bias:bound}
There exist functions $c_k$, $0\leqslant k \leqslant n$, such that for all $0\leqslant k \leqslant n$, $\parvec$, $\parvar$, all bounded measurable functions $h$ on $\mathbb{R}^d\times\mathbb{R}^d$, %and $x \in \Xset_k$,   
%$$
%\left|\int m_{\parvar,n}(x,x_{k+1}) h (x_{k+1})dx_{k+1}- \int\qg{k,\parvec}(x,x_{k+1}) h (x_{k+1})dx_{k+1}\right| \leqslant c_k(\parvec,\parvar) \| h_k \|_\infty. 
%$$
%The previous assumption should be replaced by
$$
\left|\post{k}^{\parvar}Q_{\parvar,k-1} h - \frac{\post{k-1}^{\parvar} \uk{k-1,\parvec}h}{\post{k-1}^{\parvar} \uk{k-1,\parvec}\1}\right| \leqslant c_k(\parvec,\parvar) \| h \|_\infty. 
$$ 
...
\end{hypH}
%\begin{remark}
%In the linear and Gaussian case, $\qg{k,\parvec}(x,x_{k+1}) = \Psi_{Ax+b,\Sigma}(x_{k+1})\Psi_{Cx_{k+1}+d,\Omega}(Y_{k+1})$ where $\Psi_{\mu,\lambda}$ is the Gaussian probability function with mean $\mu$ and variance $\Lambda$. Assume that $q_{\parvar,k}$ is the backward kernel of a linear and Gaussian model with parameters $\tilde A$, $\tilde b$, $\tilde \Sigma$, $\tilde C$ $\tilde d$, $\tilde \Omega$. Therefore,
%\begin{multline*}
%m_{\parvar,k}(x_{k},x_{k+1}) =   \frac{\post{k+1}^{\parvar}(x_{k+1})q_{\parvar,k}(x_{k+1},x_k)}{\post{k}^{\parvar}(x_k)} =   \frac{\post{k+1}^{\parvar}(x_{k+1})\post{k}^{\parvar}(x_k)\Psi_{\tilde Ax_k+\tilde b,\Sigma}(x_{k+1})}{\post{k}^{\parvar}(x_k)\int \post{k}^{\parvar}(x)\Psi_{Ax+b,\Sigma}(x_{k+1})d x}\\
%= \frac{\post{k+1}^{\parvar}(x_{k+1})\Psi_{\tilde Ax_k+\tilde b,\Sigma}(x_{k+1})}{\int \post{k}^{\parvar}(x)\Psi_{\tilde Ax+\tilde b,\tilde \Sigma}(x_{k+1})d x} = \frac{\Psi_{\mu_{k+1},\Sigma_{k+1}}(x_{k+1})\Psi_{\tilde Ax_k+\tilde b,\tilde \Sigma}(x_{k+1})}{\Psi_{\tilde A \mu_k+ \tilde b,\tilde\Sigma}(x_{k+1})}, 
%\end{multline*}
...
%$$
%\left|\int \left(m_{\parvar,n}(x,x_{n+1})-\qg{n,\parvec}(x,x_{n+1})\right) h (x_{n+1})dx_{n+1}\right| 
%$$
%\end{remark}
%\textcolor{red}{
%\begin{itemize}
%\item In the case of HMM/ICA models, etc. the kernels depend on the observations. Then, the upper bound $c(\parvec,\parvar)$ is random and depend on the observations. This should be detailed in the specific case of HMM.
%\item The joint probability distribution $\post{0:n}^{\parvar}$ can also be written:
%$$
% \post{0:n}^{\parvar}(x_{0:n})=  \post{0}^{\parvar}(x_0)\prod_{k=0}^{n-1}m_{\parvar,k}(x_{k},x_{k+1})\eqsp,
%$$
%where $m_{\parvar,k}(x_{k},x_{k+1}) =   \post{k+1}^{\parvar}(x_{k+1})q_{\parvar,k}(x_{k+1},x_k)/ \post{k}^{\parvar}(x_k)$. We could discuss other assumptions such as 
%$$
%\left|\int m_{\parvar,n}(x,x_{k+1}) h (x_{k+1})dx_{k+1}- \int\qg{k,\parvec}(x,x_{k+1}) h (x_{k+1})dx_{k+1}\right| \leqslant c_k(\parvec,\parvar) \| h_k \|_\infty. 
%$$
%But less useful in practice as we directly parameterize the backward variational kernel and not $ m_{\parvar,n}(x,x_{k+1}) $.
%\end{itemize}}
...
%        &\leq 2 c(\parvec,\parvar) \frac{\udup}{\udlow^2} \sum_{k = 0}^{n - 1} \| \addf{k} \|_\infty \left( \sum_{m = 1}^{n - 1} \rho^{|k - m| - 1} + 1 \right) \\
%        &\leq 2 c(\parvec,\parvar) n \frac{\udup}{\udlow^2} \left( 1 + \frac{1}{\rho} + \frac{2}{1 - \rho} \right) \sup_{0 \leq k \leq n - 1} \| \addf{k} \|_\infty\eqsp,
%    \end{align*}
$$
        \big| \postmod{0:n} h_{0:n} -  \post{0:n}^{\parvec} h_{0:n} \big| 
        \leq 2\frac{\sigma_+}{\sigma_-^2}\sum_{k=0}^{n-1}\left\| \addf{k}\right\|_\infty\left( c_0(\parvec,\parvar) + \sum_{m=1}^n \rho^{|k-m|-1}c_m(\parvec,\parvar)\right)\eqsp,%\upsilon\sup_{0 \leq k \leq n - 1}\!\!\!\! \| \addf{k} \|_\infty  c(\parvec,\parvar) n\eqsp.
$$
...
%\textcolor{red}{Add here explicit computations of $c(\parvec,\parvar)$ in the ICA case.}
\begin{itemize}
\item
...
\begin{figure}[h]
    ...
    \caption{Computation of $\big| \postmod{0:n} h_{0:n} -  \post{0:n}^{\parvec} h_{0:n} \big|$ for growing values of $n$ when both $p^\parvec$ and $q_\parvar$ are linear gaussian HMMs}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=10cm]{marginal_smoothing_errors.png}
    \caption{Computation of $\big| \postmod{0:n} \tilde{h}_{k} -  \post{0:n}^{\parvec} \tilde{h}_{k} \big|$ for different $k$ when both $p^\parvec$ and $q_\parvar$ are linear gaussian HMMs}
\end{figure}
...
\begin{equation} \label{eq:def:uk:products}
    \uk{m, n}{\parvec}(x_{0:m}', \rmd x_{0:n + 1}) \eqdef \delta_{x_{0:m}'}(\rmd x_{0:m}) \prod_{\ell = m}^n \uk{\ell}{\parvec}(x_\ell, \rmd x_{\ell + 1}) 
\end{equation}
on $(\mathbb{R}^d)^{m+1} \times \mathcal{B}((\mathbb{R}^d)^{n + 1})$, with the convention $\uk{n, n - 1}{\parvec} = \operatorname{id}$.  This yields the following decomposition:
\begin{multline*}
\postmod{0:n} \addf[n]{k} - \post{0:n}^\parvec \addf[n]{k} = 
\sum_{m = 1}^n 
\left( 
\frac{\postmod{0:m} \uk{m, n - 1}{\parvec} \addf[n]{k}}{\postmod{0:m} \uk{m, n - 1}{\parvec} \1_{}}
- \frac{\postmod{0:m - 1} \uk{m - 1, n - 1}{\parvec} \addf[n]{k}}{\postmod{0:m - 1} \uk{m - 1, n - 1}{\parvec} \1_{}} 
\right) \\ 
+ \frac{\postmod{0} \uk{0, n - 1}{\parvec} \addf[n]{k}}{\postmod{0} \uk{0, n - 1}{\parvec} \1_{}}
- \frac{\chi^\parvec \md{0}{\parvec}\uk{0, n - 1}{\parvec} \addf[n]{k}}{\chi^\parvec\md{0}{\parvec}\uk{0, n - 1}{\parvec} \1_{}},
\end{multline*}
where $\postmod{0} = \postmod{0:0}$, and since $\chi^\parvec \md{0}{\parvec} \uk{0, n - 1}{\parvec} \addf[n]{k}/\chi^\parvec\md{0}{\parvec}\uk{0, n - 1}{\parvec} \1_{} = \post{0:n}^\parvec \addf[n]{k}$.
...
%Define similarly, 
%$$
%\bkw{m}^\parvar(x_{m + 1}, \rmd x_m) = q_{\parvar,m}(x_{m+1},x_m)\mu(\rmd x_m)
%$$
%on $\mathbb{R}^d \times \mathcal{B}(\mathbb{R}^d)$. 
In addition, define for each $n \in \nsetpos$, the Markov kernel  
$$
\tstat{n}^\parvar(x_n, \rmd x_{0:n - 1}) = \prod_{m = 0}^{n - 1} Q_{\parvar,m}(x_{m + 1}, \rmd x_m)
$$ 
...
$$
\frac{\postmod{0:m} \uk{m, n - 1}{\parvec} \addf[n]{k}}{\postmod{0:m} \uk{m, n - 1}{\parvec} \1_{}} - \frac{\postmod{0:m - 1} \uk{m - 1, n - 1}{\parvec} \addf[n]{k}}{\postmod{0:m - 1} \uk{m - 1, n - 1}{\parvec} \1_{}} 
...
\postmod{0:n} \addf[n]{k} - \post{0:n}^\parvec \addf[n]{k} = 
\sum_{m = 1}^n \left( \frac{\postmod{m} \retrokmod_{m, n} \addf[n]{k}}{\postmod{m} \retrokmod_{m, n} \1_{}} - \frac{\postmod{m - 1} \retrokmod_{m - 1, n} \addf[n]{k}}{\postmod{m - 1} \retrokmod_{m - 1, n} \1_{}} \right) \\ + \frac{\postmod{0} \uk{0, n - 1}{\parvec} \addf[n]{k}}{\postmod{0} \uk{0, n - 1}{\parvec} \1_{}} - \frac{\chi^\parvec \md{0}{\parvec}\uk{0, n - 1}{\parvec} \addf[n]{k}}{\chi^\parvec\md{0}{\parvec}\uk{0, n - 1}{\parvec} \1_{}}.
\end{multline}

\paragraph{Error term at time $m=0$ in \eqref{eq:error:decomp}. }Consider the following decomposition of the first term:
\begin{align*}
\frac{\postmod{0} \uk{0, n - 1}{\parvec} \addf[n]{k}}{\postmod{0} \uk{0, n - 1}{\parvec} \1_{}} - \frac{\chi^\parvec \md{0}{\parvec}\uk{0, n - 1}{\parvec} \addf[n]{k}}{\chi^\parvec\md{0}{\parvec}\uk{0, n - 1}{\parvec} \1_{}} &= \frac{\postmod{0} \uk{0, n - 1}{\parvec} \addf[n]{k}}{\postmod{0} \uk{0, n - 1}{\parvec} \1_{}} - \frac{\phi^\parvec_0\uk{0, n - 1}{\parvec} \addf[n]{k}}{\phi^\parvec_0\uk{0, n - 1}{\parvec} \1_{}}\eqsp,\\
& = \frac{\postmod{0} \uk{0, n - 1}{\parvec} \addf[n]{k}-\phi^\parvec_0\uk{0, n - 1}{\parvec} \addf[n]{k}}{\postmod{0} \uk{0, n - 1}{\parvec} \1_{}} \\
&\hspace{2cm}+  \frac{\phi^\parvec_0\uk{0, n - 1}{\parvec} \addf[n]{k}}{\phi^\parvec_0\uk{0, n - 1}{\parvec} \1_{}}\frac{\phi^\parvec_0\uk{0, n - 1}{\parvec} \1_{} - \postmod{0} \uk{0, n - 1}{\parvec} \1_{}}{\postmod{0} \uk{0, n - 1}{\parvec} \1_{}} \eqsp,
\end{align*}
...
$$
\left|\frac{\postmod{0} \uk{0, n - 1}{\parvec} \addf[n]{k}}{\postmod{0} \uk{0, n - 1}{\parvec} \1_{}} - \frac{\phi^\parvec_0\uk{0, n - 1}{\parvec} \addf[n]{k}}{\phi^\parvec_0\uk{0, n - 1}{\parvec} \1_{}}\right|\leqslant 2c_0(\parvec,\parvar)\frac{\| \uk{0, n - 1}{\parvec} \1_{}\|_\infty\|\addf[n]{k}\|_\infty}{\postmod{0} \uk{0, n - 1}{\parvec} \1_{}}\eqsp.
$$
By H\ref{assum:strong:mixing}, for all $x_0\in \mathbb{R}^d$,
$$
\uk{0, n - 1}{\parvec} \1_{}(x_0) = \int  \ud{0,\parvec}(x_0, x_{1})\mu(\rmd x_1)\uk{1, n - 1}{\parvec} \1_{}(x_1)\leqslant \sigma_+  \int \mu(\rmd x_1)\uk{1, n - 1}{\parvec} \1_{}(x_1)
$$
and 
$$
\postmod{0} \uk{0, n - 1}{\parvec} \1 = \int \postmod{0} (\rmd x_0)\ud{0,\parvec}(x_0, x_{1})\mu(\rmd x_1)\uk{1, n - 1}{\parvec} \1_{}(x_1)\geqslant \sigma_-  \int \mu(\rmd x_1)\uk{1, n - 1}{\parvec} \1_{}(x_1)\eqsp,
$$
which yields
$$
\left|\frac{\postmod{0} \uk{0, n - 1}{\parvec} \addf[n]{k}}{\postmod{0} \uk{0, n - 1}{\parvec} \1_{}} - \frac{\phi^\parvec_0\uk{0, n - 1}{\parvec} \addf[n]{k}}{\phi^\parvec_0\uk{0, n - 1}{\parvec} \1_{}}\right|\leqslant 2c_0(\parvec,\parvar)\frac{\sigma_+}{\sigma_-}\|\addf[n]{k}\|_\infty\eqsp.
$$
...
- \frac{\postmod{m - 1} \retrokmod_{m - 1, n} \addf[n]{k}}{\postmod{m - 1} \retrokmod_{m - 1, n} \1_{}}  = \frac{\postmod{m} Q_{\parvar,m-1} \tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n} \addf[n]{k}}{\postmod{m} \retrokmod_{m, n} \1_{}}
...
$$
\mathcal{L}^{*,\parvar,\parvec}_{m, n}h(x_{m-1},x_m) = \frac{\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n}h(x_{m-1},x_m) }{\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n}\1(x_{m-1},x_m)} - \frac{\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n}h(x^*_{m-1},x^*_m) }{\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n}\1(x^*_{m-1},x^*_m)} = \frac{\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n}h(x_{m-1},x_m) }{\uk{m, n - 1}{\parvec}\1(x_m)} - \frac{\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n}h(x^*_{m-1},x^*_m) }{\uk{m, n - 1}{\parvec}\1(x^*_m)}\eqsp,
$$
and define the normalized measure $\tilde\phi^\parvar_mh$ by $\postmod{m - 1} \uk{m-1,\parvec}h/\postmod{m - 1} \uk{m-1,\parvec}\1$, so that
\begin{align*}
 \frac{\postmod{m} \retrokmod_{m, n} \addf[n]{k}}{\postmod{m} \retrokmod_{m, n} \1_{}} &- \frac{\postmod{m - 1} \retrokmod_{m - 1, n} \addf[n]{k}}{\postmod{m - 1} \retrokmod_{m - 1, n} \1_{}}  \\
&= \frac{\postmod{m} Q_{\parvar,m-1} \left\{\mathcal{L}^{*,\parvar,\parvec}_{m, n} \addf[n]{k}\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n} \1\right\}}{\postmod{m} \retrokmod_{m, n} \1_{}} - \frac{\postmod{m - 1} \uk{m-1}{\parvec} \left\{\mathcal{L}^{*,\parvar,\parvec}_{m, n} \addf[n]{k}\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n} \1\right\}}{\postmod{m - 1} \retrokmod_{m - 1, n} \1_{}} \\ 
&=  \frac{\postmod{m} Q_{\parvar,m-1} \left\{\mathcal{L}^{*,\parvar,\parvec}_{m, n} \addf[n]{k}\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n} \1\right\}}{\postmod{m} \retrokmod_{m, n} \1_{}} - \frac{\tilde\phi^\parvar_m\left\{\mathcal{L}^{*,\parvar,\parvec}_{m, n} \addf[n]{k}\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n} \1\right\}}{\tilde\phi^\parvar_m\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n}\1_{}} \\ \\
&= \frac{\postmod{m} Q_{\parvar,m-1} \left\{\mathcal{L}^{*,\parvar,\parvec}_{m, n} \addf[n]{k}\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n} \1\right\} - \tilde\phi^\parvar_m \left\{\mathcal{L}^{*,\parvar,\parvec}_{m, n} \addf[n]{k}\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n} \1\right\}}{\tilde\phi^\parvar_m\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n}\1_{}} \\
&\hspace{2.5cm}+  \frac{\postmod{m} Q_{\parvar,m-1}\left\{\mathcal{L}^{*,\parvar,\parvec}_{m, n} \addf[n]{k}\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n} \1\right\}}{\postmod{m} \retrokmod_{m, n} \1_{}}\left(\frac{\tilde\phi^\parvar_m\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n}\1-\postmod{m} \retrokmod_{m, n} \1_{}}{\tilde\phi^\parvar_m\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n}\1_{}}\right)\eqsp.
\end{align*}
Then, using that
$$
\left|\frac{\postmod{m} Q_{\parvar,m-1} \left\{\mathcal{L}^{*,\parvar,\parvec}_{m, n} \addf[n]{k}\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n} \1\right\}}{\postmod{m} \retrokmod_{m, n} \1_{}}\right|\leqslant \left\|\mathcal{L}^{*,\parvar,\parvec}_{m, n} \addf[n]{k}\right\|_\infty\eqsp,
$$
...
\left|\frac{\postmod{m} Q_{\parvar,m-1} \left\{\mathcal{L}^{*,\parvar,\parvec}_{m, n} \addf[n]{k}\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n} \1\right\} - \tilde\phi^\parvar_m\left\{\mathcal{L}^{*,\parvar,\parvec}_{m, n} \addf[n]{k}\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n} \1\right\}}{\tilde\phi^\parvar_m\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n} \1_{}}\right| &\leqslant c_m(\parvec,\parvar)\frac{\left\|\mathcal{L}^{*,\parvar,\parvec}_{m, n} \addf[n]{k}\right\|_\infty\left\|\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n} \1\right\|_\infty}{\tilde\phi^\parvar_m\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n}\1_{}}\eqsp,
\end{align*}
yields
$$
...
$$
\tilde\phi^\parvar_m\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n}\1_{} \geqslant \sigma_-^2 \mu  \uk{m+1, n-1}{\parvec}\1\eqsp,
$$ 
...
%\begin{multline*}
%\postmod{0:n} \addf[n]{k} - \post{0:n} \addf[n]{k} = \frac{\postmod{0} \uk{0, n - 1}{\parvec} \addf[n]{k}}{\postmod{0} \uk{0, n - 1}{\parvec} \1_{\Xset^n}} - \frac{\chi \uk{0, n - 1}{\parvec} \addf[n]{k}}{\chi\uk{0, n - 1}{\parvec} \1_{\Xset^n}} + \sum_{m = 1}^k \left( \noshift_{m, n} \retrokmodnorm_{m, n} \addf[n]{k} - \shiftbwd_{m, n}  \retrokmodnorm_{m, n} \addf[n]{k} \right) \\ 
...
%$$
%\left| \postmod{0:n} \addf[n]{k} - \post{0:n}^\parvec \addf[n]{k} \right| \leq 2 c(\parvec,\parvar) \frac{\udup }{\udlow^2} \sum_{k = 0}^{n - 1} \| \addf{k} \|_\infty \left( \sum_{m = 1}^{n - 1} \rho^{|k - m| - 1} + 1 \right),
%$$
...
$$
\frac{\lambda \tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n} \addf[n]{k}}{\lambda \tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n} \1_{}} = \frac{\lambda \prod_{\ell = k}^{m-2}Q_{\parvar,\ell}\addf{k}\uk{m, n-1}{\parvec}\1_{}}{\lambda(\1\otimes \uk{m, n-1}{\parvec}\1_{})} = \lambda_{m,n}Q_{\parvar,m-2}\ldots Q_{\parvar,k}\addf{k}\eqsp,
$$
...
$$
\eta_{n,m}f = \frac{\eta (f\otimes \uk{m, n-1}{\parvec}\1_{})}{\eta (\1\otimes \uk{m, n-1}{\parvec}\1_{})}\eqsp.
$$
Therefore,
$$
...
$$
\frac{\lambda\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n} \addf[n]{k}}{\lambda\tilde{\mathcal{L}}^{\parvar,\parvec}_{m, n} \1_{}}= \frac{\lambda(1\otimes F^\parvec_{m|n}\ldots F^\parvec_{k|n}\addf[n]{k}\cdot\uk{m, n-1}{\parvec}\1_{})}{\lambda (\1\otimes  \uk{m, n-1}{\parvec}\1_{})}\eqsp, 
$$
...
$$
\eta_{m|n}h = \frac{\eta(1\otimes h\uk{m, n-1}{\parvec}\1_{})}{\eta(1\otimes \uk{m, n-1}{\parvec}\1_{})}\eqsp.
$$
This yields
$$
...
%$$
%\left| \noshift_{m, n} h - \shiftbwd_{m, n} h \right| \vee \left| \shiftfwd_{m, n} h - \noshift_{m, n} h \right| \leq 2 c(\parvec,\parvar)  \frac{\udup}{\udlow^2} \| h \|_\infty.   
%$$
%\item[(ii)] For all $n \in \nset$, $m \in \intvect{0}{n - 1}$, and $\precpar \in \precparsp$,
%$$
%\left| \frac{\postmod{m + 1} \retrokmod_{m + 1, n} \addf[n]{m}}{\postmod{m + 1} \retrokmod_{m + 1, n} \1_{\Xset^n}} - \frac{\postmod{m} \retrokmod_{m, n} \addf[n]{m}}{\postmod{m} \retrokmod_{m, n} \1_{\Xset^n}} \right| \leq 2 c(\parvec,\parvar)  \frac{\udup}{\udlow^2} \| h_m \|_\infty. 
%$$
%\end{itemize}
%In both cases, $c(\parvec,\parvar)$ is the constant in H
...
%\begin{multline*}
% \noshift_{m, n} h - \shiftbwd_{m, n} h = \parvar_{m, n}^\precpar h \left( \frac{\postmod{m - 1} \uk{m - 1} \retrokmod_{m, n} \1_{\Xset^n} - \postmod{m - 1} \ukmod{m - 1} \retrokmod_{m, n} \1_{\Xset^n}}{\postmod{m - 1} \retrokmod_{m - 1, n} \1_{\Xset^n}} \right) \\
...
%$$
%\left| \noshift_{m, n} h - \shiftbwd_{m, n} h \right| \leq 2 c(\parvec,\parvar) \frac{\| \retrokmod_{m, n} \1_{\Xset^n} \|_\infty  \| h \|_\infty}{\postmod{m - 1} \retrokmod_{m - 1, n} \1_{\Xset^n}}.
%$$
...
%$| \noshift_{m, n} h - \shiftbwd_{m, n} h| \leq 2 c(\parvec,\parvar) \udup \| h \|_\infty / \udlow^2
%$. 
...
%
% For all $n\in\nset$, write $\post{0:n}^{\parvar}$ the joint smoothing distribution provided by the variational family. Then,
%$$
%\post{0:n}^\parvec h - \post{0:n}^{\parvar} h= \post{n}^\parvec \tstat{n}^\parvec h - \post{0:n}^{\parvar} h =  \post{n}^\parvec \left\{\prod_{m = 0}^{n - 1} \bkw{m}^\parvec\right\} h - q_{\parvar,n}\left\{\prod_{k=0}^{n-1}q_{\parvar,k}\right\} h\eqsp.
%$$
%Write 
%$$
%\postmod{0:n} h_n - \post{0:n}^{\parvar} h_n = \sum_{k = 0}^{n - 1} \left( \postmod{0:n} \addf[n]{k} - \post{0:n}^{\parvar} \addf[n]{k} \right), 
%$$
%where each term can be decomposed according to   
%\begin{equation*}
%\postmod{0:n} \addf[n]{k} - \post{0:n}^\parvar \addf[n]{k} = 
...
%\begin{align}
%    p^\parvec(u_1^{(i)}) &= \prod_{k=1}^K (\pi_k^{(i)})^{\delta(u_1^{(i)} = k)} \\
%    p^\parvec(u_t^{(i)}\mid u_{t-1}^{(i)}) &= \prod_{k=1}^K\prod_{\ell=1}^K (A_{k\ell}^{(i)})^{ \delta(u_t^{(i)}=k)\delta(u_{t-1}^{(i)}=\ell)} \\
%    p^\parvec( y_1^{(i)}\mid u_1^{(i)}) &= \prod_{k=1}^K\mathcal{N}( y_1^{(i)} ; \bar{ b}_k^{(i)}, \bar{Q}_k^{-1^{(i)}})^{\delta(u_1^{(i)}=k)} \\
%    p^\parvec( y_t^{(i)}\mid  y_{t-1}^{(i)}, u_t^{(i)}) &= \prod_{k=1}^K \mathcal{N}( y_t^{(i)} ; B_k^{(i)}  y_{t-1}^{(i)} +  b_k^{(i)}, Q_k^{-1^{(i)}})^{\delta(u_t^{(i)}=k)} \\
%    p^\parvec( x_t\mid  s_t) &= \mathcal{N}( x_t ;  f^\parvec(s_t),  R^{-1})
%\end{align}
%where the superscript $(i)$ again denotes that each independent component $i \in \{1,\dots, N\}$ follows its own switching linear dynamical system. Also, each independent component is part of a higher dimensional latent component $ y_t^{(i)} = (s_t^{(i)}, y_{t, 2}^{(i)},\dots, y_{t, d}^{(i)})$. The mixing function $f^\parvec$ and other variables are defined as in the main text. The complete loglikelihood is defined as:
%\begin{align}
%	\log p^\parvec( x_{1:T}^{(1:N)},  y_{1:T}^{(1:N)}, u_{1:T}^{(1:N)}) &= \sum_{t=1}^T \log p^\parvec( x_t \mid  s_t) + \sum_{i=1}^N\left(\log p^\parvec(u_1^{(i)}) +\log p^\parvec( y_1^{(i)} \mid u_1^{(i)}) \right. \nonumber \\
%	&\left. \sum_{t=2}^T \log p^\parvec(u_t^{(i)}\mid u_{t-1}^{(i)}) + \log p^\parvec( y_t^{(i)} \mid  y_{t-1}^{(i)}, u_t^{(i)}) \right) \eqsp.
%\end{align}
...
%&= \pE_{q_\phi}\bigg[\sum_{t=1}^T\log p^\parvec( x_t\mid s_t^{(1:N)})\bigg]  + \pE_{q_\phi}\left[\log \frac{p_\parvec(y_{1:T}^{(1:N)}, u_{1:T}^{(1:N)})}{q_\phi(y_{1:T}^{(1:N)}, u_{1:T}^{(1:N)})}\right]\\
%&= \pE_{q_\phi}\bigg[\sum_{t=1}^T\log p^\parvec( x_t\mid s_t^{(1:N)})\bigg]  + \sum_{i=1}^n\left\{\pE_{q_\phi}\left[\log p_\parvec(y_{1}^{(i)}, u_{1}^{(i)})\right] + \sum_{t=1}^{T-1}\pE_{q_\phi}\left[\log p_\parvec(y_{t+1}^{(i)}, u_{t+1}^{(i)}|y_{t}^{(i)}, u_{t}^{(i)})\right]\right\}\\
...
%\mathcal{L}(\parvec,\phi)  = \E_{q_\phi}\bigg[\log \frac{p^\parvec( x_{1:T},  y_{1:T}^{(1:N)}, u_{1:T}^{(1:N)})}{q_\phi( y_{1:T}^{(1:N)}, u_{1:T}^{(1:N)})} \bigg] = \E_{q_\phi}\left[V_T^{\parvec,\phi}( u^{(1:N)}_T,y^{(1:N)}_T)\right]\nonumber
%\end{equation}
%where
%\begin{align}
%V_T^{\parvec,\phi}( u^{(1:N)}_T,y^{(1:N)}_T) &= \E_{q_\phi}\left[\log \frac{p^\parvec( x_{1:T},  y_{1:T}^{(1:N)}, u_{1:T}^{(1:N)})}{q_\phi( y_{1:T}^{(1:N)}, u_{1:T}^{(1:N)})}\middle| u^{(1:N)}_T,y^{(1:N)}_T \right]\\
%&= \ldots
%\end{align}
%%\begin{align}
%%\mathcal{L}(\parvec,\phi)  &= \E_{q_\phi}\bigg[\log \frac{p^\parvec( x_{1:T},  y_{1:T}^{(1:N)}, u_{1:T}^{(1:N)})}{q_\phi( y_{1:T}^{(1:N)}, u_{1:T}^{(1:N)})} \bigg] \nonumber\\
%%    &= \E_{q_\phi}\bigg[\sum_{t=1}^T\log p( x_t\mid s_t^{(1)}, \ldots,  s_t^{(N)}) + \sum_{i=1}^N\log\frac{p^\parvec( y_{1:T}^{(i)}\mid u_{1:T}^{(i)}) p^\parvec(u_{1:T}^{(i)})}{q_\phi( y_{1:T}^{(i)},u_{1:T}^{(i)})} \bigg] \nonumber\\
%%    &= \E_{q_\phi}\bigg[\sum_{t=1}^T\log p^\parvec( x_t\mid s_t^{(1)}, \ldots,  s_t^{(N)})\bigg] + \sum_{i=1}^N\Bigg(-\mathrm{KL}\bigg[q(u_{1:T}^{(i)})\bigg| p(u_{1:T}^{(i)})\bigg] + \mathrm{H}\bigg[q( y_{1:T}^{(i)})\bigg] \nonumber\\    
%%    &\qquad+ \E_{q}\bigg[\log p( y_{1:T}^{(i)}\mid u_{1:T}^{(i)})\bigg]\Bigg) \nonumber\\
%%    &= \E_{q_\phi}\bigg[\sum_{t=1}^T\log p^\parvec( x_t\mid s_t^{(1)}, \ldots,  s_t^{(N)})\bigg] + \sum_{i=1}^N\Bigg(- \mathrm{KL}\bigg[q(u_{1:T}^{(i)})\bigg| p(u_{1:T}^{(i)})\bigg] + \mathrm{H}\bigg[q( s_{1:T}^{(i)})\bigg] \nonumber\\
